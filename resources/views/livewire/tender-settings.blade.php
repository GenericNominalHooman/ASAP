            ...
            <div class="p-6 text-gray-900 dark:text-gray-100">
                <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <form class="max-w-sm mx-auto" wire:submit.prevent="saveSettings">
                        @csrf
                        <!-- User's Automatic Application Timer settings -->
                        <h2 class="text-xl font-bold mb-2">Automatic Quotations Apply: </h2>
                        <p class="text-sm text-gray-500 mb-4 italic">At what time of the day do you want to auto apply for quotations? - if all is enabled, the system will auto apply 3 times a day</p>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                @foreach($quotationApplicationTimerOptions as $timing)
                                    <div class="flex items-center">
                                        <input 
                                            type="checkbox" 
                                            id="timer_{{ Str::slug($timing) }}"
                                            wire:model="selectedTimers"
                                            value="{{ $timing }}"
                                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                                        >
                                        <label for="timer_{{ Str::slug($timing) }}" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                                            {{ $timing }}
                                        </label>
                                    </div>
                                @endforeach
